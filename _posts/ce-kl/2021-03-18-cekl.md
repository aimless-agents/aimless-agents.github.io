...
We wanted to dedicate an entire post to the lovely functions cross entropy and Kullback-Leibler divergence, which are very widely used in training ML models but often hard to understand.
...
Cross entropy is a great loss function to use for most **multi-class classification** problems. This describes problems like our weather-predicting example: you have 2+ different classes you’d like to predict, and the classes are mutually exclusive (each data point can only belong to one class). The cross entropy we’ve defined in this section, and what people are usually referring to when they say “cross entropy”, is specifically **categorical cross entropy**. 
...
KL divergence is fairly easy to understand once you’ve got cross entropy down: it’s just the difference between cross entropy and the true distribution’s entropy. 
$$KL(p, q) = H(p, q) - H(p)$$
